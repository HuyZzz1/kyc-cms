  ...
  return (
    ...
    {data.map((item, index) =>
      <li className={classNames({'nk-menu-item': true, 'has-sub' : item.subMenu})} key={index}>
        {!item.subMenu ? (
          <NavLink to={item.link} className="nk-menu-link" target={item.newTab && '_blank'} end>
                ...
                <Fragment key={sIndex}>
                  {sItem.heading ? (
                    ...
                    <li className={classNames({'nk-menu-item': true, 'has-sub' : sItem.subMenu})} >
                        {!sItem.subMenu ? (
                          <NavLink to={sItem.link} className="nk-menu-link" target={sItem.newTab && '_blank'} end>
                              ...
                              <ul className="nk-menu-sub">
                                {sItem.subMenu.map((s2Item, s2Index) =>
                                  <li className={classNames({'nk-menu-item': true, 'has-sub' : s2Item.subMenu})} key={s2Index}>
                                      {!s2Item.subMenu ? (
                                        <NavLink to={s2Item.link} className="nk-menu-link" target={s2Item.newTab && '_blank'} end>
                                            ...
                                            <ul className="nk-menu-sub">
                                              {s2Item.subMenu.map((s3Item, s3Index) =>
                                                <li className="nk-menu-item" key={s3Index}>
                                                    <NavLink to={s3Item.link} className="nk-menu-link" target={s3Item.newTab && '_blank'} end>
